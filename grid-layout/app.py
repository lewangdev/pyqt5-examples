...
class MainUi(QtWidgets.QMainWindow):
    ...
    def init_ui(self):
        ...
        # set styles
        self.left_close.setFixedSize(14, 14)  # 设置关闭按钮的大小
        self.left_visit.setFixedSize(14, 14)  # 设置按钮大小
        self.left_mini.setFixedSize(14, 14)  # 设置最小化按钮大小

        self.left_close.setStyleSheet('''QPushButton{background:#F76677;border-radius:7px;}QPushButton:hover{background:red;}''')
        self.left_visit.setStyleSheet('''QPushButton{background:#F7D674;border-radius:7px;}QPushButton:hover{background:yellow;}''')
        self.left_mini.setStyleSheet('''QPushButton{background:#6DDF6D;border-radius:7px;}QPushButton:hover{background:green;}''')
...
''')
    ...
    def mousePressEvent(self, event):
        if event.button() == Qt.LeftButton:
            ...
            self.move_offset = event.globalPos() - self.pos()  # 获取鼠标相对窗口的位置
            event.accept()
            self.setCursor(QCursor(Qt.OpenHandCursor))         # 更改鼠标图标

    def mouseMoveEvent(self, event):
        if Qt.LeftButton and self.left_button_pressed:
            self.move(event.globalPos() - self.move_offset)  # 更改窗口位置
            event.accept()
